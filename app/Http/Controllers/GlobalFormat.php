...
class GlobalFormat extends Controller
{
    ...
    public function format_products($products) {
        ...
        foreach($products as $key => $product){
            ...
            $data[$key]['desc'] = $product->product_desc;
            ...
            $data[$key]['discounted_price'] = $currency.number_format($data[$key]['discounted_price']);
            $percentage = ( $product->product_discount / $product->product_price ) * 100;
            $data[$key]['discounted_percentage'] = round($percentage);
        }
